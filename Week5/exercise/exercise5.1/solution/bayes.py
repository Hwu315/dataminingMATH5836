...
def main():
	...
	for problem in range(1, 3): 
		...
		if problem == 1:  # you can do similarly for COVID-19 daily infection prediction. Process and change the data folder
			...
			activation = False  # true for sigmoid, false for linear regression - can be either since data is between [0,1]
# 
		if problem == 2:
			normalise = True
		...
		plt.savefig(str(problem)+'mcmctest.png') 
		plt.clf()
		# -----------------------------------------
		...
		plt.savefig(str(problem)+'mcmctrain.png') 
		plt.clf()
		...
		plt.title("Posterior")
		plt.savefig(str(problem)+'w_pos.png') 
